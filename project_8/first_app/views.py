    ...
    if not request.user.is_authenticated:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                user = authenticate(username = name, password=userpass) #checking if user exists in database
...
def pass_change(request):
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            if form.is_valid():
                form.save()
                update_session_auth_hash(request, form.user) #pass update
                return redirect('profile')
        else:
